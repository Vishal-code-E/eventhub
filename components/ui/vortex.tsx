...
interface VortexProps {
  ...
  className?: string;            // styles for the foreground content wrapper
  containerClassName?: string;   // styles for the outer container
...
export const Vortex = ({
  ...
  const draw = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
    ...
    ctx.fillStyle = backgroundColor;      // pure black background
  ...
  const drawParticle = (
    ...
    // still monochrome-ish, but you can set hue to 0 for pure white
    ctx.strokeStyle = `hsla(${hue}, 0%, 100%, ${fadeInOut(life, ttl)})`;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x2, y2);
    ctx.stroke();
  ...
  <div className={cn("relative h-full w-full", containerClassName)}>
    ...
    {/* Just overlay whatever children we pass */}
